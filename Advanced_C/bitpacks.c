...
//this serves as an exercise of storing data in bits in a single integer
{
    ...
    bitStatus =
            (num >> position) & 1; //we get the bit we wanna change at the start and & it with 1, which is 1 in binary
    printf("The %d bit is set to %d\n", position, bitStatus);

    //Left shift 1, n times and perform bitwise OR with num
    newNum = (1 << position) |
             num; //we left shift on 1 which is ..0001 in binary, and then OR to change the bit in number to 1
