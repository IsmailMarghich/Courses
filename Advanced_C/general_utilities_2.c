...
int main() {
    ...
    double doublepoint = atof(string);
    (string); //atof converts ascii to double
    ...
    memmove(secondarray, array,
            SIZE * sizeof(int)); //memmove is similar to memcpy, but with memcpy the memory cells cant overlap
