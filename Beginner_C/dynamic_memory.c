...
int main ( )
{   //with malloc we can manually allocate bytes to the variable, here we use 25 * the size of an integer, to get memory
    ...
    if (!p1Number) //we use an if statement to make sure the memory is allocated
    {
        ...
        p2Number = (int*)realloc(p2Number, 50); //with realloc we can add additional memory, where the second argument is how many bytes is to be added
        free(p2Number);
        p2Number = NULL;
    }
