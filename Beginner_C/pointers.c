...
int lenString(const char *str) {
    ...
    return lastAdress -
           str; //then substract the end pointer with start pointer (if u call an array in a pointer it goes to first pointer) which returns the length

}

void swapint(int *a,
             int *b) //this function will swap integers with their pointers, as this will edit their value in the main program
