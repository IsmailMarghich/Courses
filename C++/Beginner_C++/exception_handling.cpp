...
int main() {
    ...
    double avrg = exception_function(10, 2); //change the second argument to zero, to see a different way of exception handling
    cout << "Average: " << avrg << endl;
}
//we can also catch exceptions with functions
double exception_function(int sum, int total) {
    if (total == 0)
        throw runtime_error("Cannot divide by zero, please enter a total above 0"); //we can also use throw to handle exceptions
    return static_cast<double>(sum) / total;                                         //this is however limited, because we need to use if statements
}