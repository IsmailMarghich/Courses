...
public:
    ...
    Player(string name_val, int health_val,
           int xp_val) //we prototype our constructor, then after a colon we initialize the fields, separated by commas
            : name(name_val), health(health_val),
              xp(xp_val) {} //this way, we can immediately initialize class variables when we get them
