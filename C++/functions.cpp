...
void changenumber(
        int &number) { //we use & to get to the address of the variable, instead of making a copy, it changes now the actual variable in main
...
void printarray(const int numbers[], size_t size) {
    for (size_t i{0}; i <
                      size; ++i) //for loop to print the whole array, we have to use size_t as its the type for memory sizes in C++
        cout << numbers[i] << endl;
}

int recursivefunction(int n) { //this function will calculate the sum of n amount of numbers in the fibonacci series
    if (n <=
        0) //theres no negative numbers in the fibonacci series, so we return zero if we reach zero or negative number
        return 0;
    else if (n == 1) //if we reach the first number, we return 1 since the fibonacci series stars at 1
        return 1;
    return recursivefunction(n - 1) + recursivefunction(
            n - 2); //otherwise we call the function to calculate the current and previous value, and add them together
