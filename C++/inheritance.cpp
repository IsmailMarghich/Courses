...
class Savings_Account
        : public Account { //we create a sub class of Account class, now it will get its variables and methods
private:
    float interest_rate; //this sub class adds an interest rate variable
public:
    void calculate_interest(
            int years) { //function to calculate how much interest you will be gaining on your savings account
        double gained = (balance * pow(((interest_rate / 100) + 1), years) -
                         balance); //this calculates the amount of money gained based on interest rate, given years
        cout << "You earned " << gained << " on " << balance << "$" << endl; //print out how much is gained
    }

    Savings_Account(int amount,
                    float interest) { //constructor to start an account, u can fill in starting balance and your interest rate
        balance = amount;
